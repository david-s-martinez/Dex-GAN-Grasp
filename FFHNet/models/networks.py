...
class FFHCollDetr(nn.Module):
    def __init__(self,
                 ...
                 dtype=torch.float32,
                 **kwargs):
        super(FFHCollDetr, self).__init__()
        self.cfg = cfg
...
class FFHCollDetrTwoResBlock(FFHCollDetr):
    def __init__(self, cfg, n_neurons=512, in_bps=4096, in_pose=9 + 3, dtype=torch.float32, **kwargs):
        super().__init__(cfg, n_neurons, in_bps, in_pose, dtype, **kwargs)
        self.rb4 = ResBlock(in_bps + in_pose + n_neurons, n_neurons)
        self.rb5 = ResBlock(in_bps + in_pose + n_neurons, n_neurons)
        self.rb6 = ResBlock(in_bps + in_pose + n_neurons, n_neurons)
    ...
    def forward(self, data):
        ...
        X = self.rb3(torch.cat([X, X0], dim=1))
        X = self.dout(X)
        X = self.rb4(torch.cat([X, X0], dim=1))
        X = self.dout(X)
        X = self.rb5(torch.cat([X, X0], dim=1))
        X = self.dout(X)
        X = self.rb6(torch.cat([X, X0], dim=1))
        X = self.dout(X)
        X = self.out_success(X)
...
class FFHCollDetrSixResBlock(FFHCollDetr):
    def __init__(self, cfg, n_neurons=512, in_bps=4096, in_pose=9 + 3, dtype=torch.float32, **kwargs):
        super().__init__(cfg, n_neurons, in_bps, in_pose, dtype, **kwargs)
