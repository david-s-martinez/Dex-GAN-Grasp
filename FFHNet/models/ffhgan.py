...
from FFHNet.models.networks import FFHGenerator,FFHEvaluator, FFHCollDetr, FFHCollDetr, FFHGAN
...
class FFHGANet(object):
    ...
    def __init__(self, cfg):
        ...
        if cfg["model"] == "ffhnet":
            self.FFHGAN, self.FFHEvaluator, self.FFHCollDetr = build_ffhnet(cfg, self.device, is_train=self.is_train)
        elif cfg["model"] == "pointnet":
            self.FFHGAN, self.FFHEvaluator, self.FFHCollDetr = build_pointnet(cfg, self.device, is_train=self.is_train)
        else:
    ...
    def update_ffhgan(self, real_data):
        """ Receives a dict with all the input data to the ffhgenerator, sets the model input and runs one complete update step.
        """
        # Make sure net is in train mode
        self.FFHGAN.train()
